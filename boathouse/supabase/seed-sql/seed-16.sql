...
from (
  select s2.id,
    case when coalesce(sum(case when a.asset_class like 'shell_%' then a.seats end), 0) >= s2.requested_seats then 'allocated'
         when count(a.id) filter (where a.asset_class like 'shell_%') > 0 then 'partially_allocated'
