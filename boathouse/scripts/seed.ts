...
// Deterministic: two runs yield identical rows and ids.
...
import { createServerClient, must, type Db } from "@/lib/supabase";
...
async function fetchAll(db: Db, table: string): Promise<Record<string, unknown>[]> {
...
export async function checksums(db: Db): Promise<{ table: string; rows: number; sha256: string }[]> {
  const out: { table: string; rows: number; sha256: string }[] = [];
...
async function seed(db: Db): Promise<void> {
  ...
  console.log("Truncating all tables in dependency order");
  const trunc = await db.rpc("truncate_all");
  if (trunc.error) throw new Error(`truncate_all: ${trunc.error.message}`);

  console.log(`Loading ${organizations.length} organizations, ${assets.length} assets, ${entitlements.length} entitlements`);
  await insertAll(db, "organization", organizations);
  // Stored status per 4.6 from the seeded damage events (event 6 and 7 are unresolved).
  ...
  console.log(`Loading ${sessions.length} sessions, ${damageEvents.length} damage events, ${parts.length} parts`);
  await insertAll(db, "session", sessions);
  await insertAll(db, "damage_event", damageEvents);
  await insertAll(db, "part", parts);

  console.log("Generating every week of the season");
  let inserted = 0;
  ...
  console.log(`Inserted ${inserted} allocations; approved weeks through ${LAST_APPROVED_MONDAY}`);
}

async function main(): Promise<void> {
  const db = createServerClient();
  const onlyChecksum = process.argv.includes("--checksum");
  if (!onlyChecksum) await seed(db);
